...
export default function Proveedores() {
  ...
  async function handleDeleteConfirm() {
    try {
      await api.delete(`/proveedores/${confirm.id}`);
  ...
  const filtrados  = items.filter(p =>
    ...
    (p.correoElectronico || '').toLowerCase().includes(buscar.toLowerCase())
  );
    ...
    <div style={{ animation: 'fadeUp 0.3s ease' }}>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Directorio de proveedores</p>
        </div>
      ...
      <div className="table-card">
        <div className="table-toolbar">
          <input className="search-input" placeholder="🔍  Buscar proveedor..." value={buscar}
        ...
        <table>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={5}><div className="empty-state"><div className="empty-state-icon">🏭</div><p>Sin proveedores registrados</p></div></td></tr>
            ) : paginados.map(p => (
              <tr key={p.idProveedor}>
                <td><span className="badge badge-blue">#{p.idProveedor}</span></td>
                <td><strong>{p.nombre}</strong></td>
                <td>{p.telefono || '—'}</td>
                <td>{p.correoElectronico || '—'}</td>
                <td><div className="td-actions">
                  <button className="btn btn-ghost btn-sm" onClick={() => handleEdit(p)}>✏️ Editar</button>
                  <button className="btn btn-danger btn-sm" onClick={() => setConfirm({ id: p.idProveedor, msg: '¿Eliminar este proveedor?' })}>🗑️</button>
                </div></td>
              </tr>
            ))}
          </tbody>
              ...
              <div className="modal-body">
                <div className="form-grid">
                  <div className="form-group full">
                    ...
                    <input className="form-input" name="nombre" value={form.nombre} onChange={handleChange} placeholder="Nombre del proveedor" autoFocus />
                  </div>
